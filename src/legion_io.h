...
public: 
  ...
  PhysicalRegion pr;
  DomainPoint dp; // for structured index spaces
  unsigned int color;  // for unstructured index spaces
  char shard_name[40];
private:
...
class PersistentRegion {
 public:
  ...
  // for unstructured index spaces 
    void create_persistent_subregions(Context ctx, const char *name, LogicalRegion parent_lr,
                                      LogicalPartition lp,
                                      Coloring coloring, std::map<FieldID, std::string> &field_map); 
